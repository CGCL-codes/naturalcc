...
def build_graph(tree_dict, dictionary, tree_leaf_subtoken=1, DGLGraph_PAD_WORD=-1) -> dgl.DGLGraph:
    ...
    def _build(nid, idx, tree):
        ...
        if not isinstance(tree[idx]['children'][1], list):
            ...
            if nid is None:
                nx_graph.add_node(0, x=[DGLGraph_PAD_WORD] * MAX_SUBTOKEN_LEN, y=int(idx), mask=0)
                # print('node={}, x={}, y={}, mask={}'.format(0, [DGLGraph_PAD_WORD] * MAX_SUBTOKEN_LEN, int(idx), 0))
                nid = 0
            for idx in child_ids:
                cid = nx_graph.number_of_nodes()
                y_value = int(idx)
                if not isinstance(tree[str(idx)]['children'][1], list):  # non-leaf node
                    nx_graph.add_node(cid, x=[DGLGraph_PAD_WORD] * MAX_SUBTOKEN_LEN, y=y_value, mask=0)
                    # print(
                    #     'node={}, x={}, y={}, mask={}'.format(cid, [DGLGraph_PAD_WORD] * MAX_SUBTOKEN_LEN, y_value, 0))
...
def tree2dgl(tree_dict, dictionary, DGLGraph_PAD_WORD=-1):
    ...
    def nonleaf_node_info():
        if _subtoken:
            return [DGLGraph_PAD_WORD] * MAX_SUBTOKEN_LEN
        else:
