...
class AMPOptimizer(optim.NccOptimizer):
    """
    Wrap an *optimizer* to support AMP (automatic mixed precision) training.
    """

    def __init__(self, args, params, fp32_optimizer, **kwargs):
        super().__init__(args)
        self.fp32_optimizer = fp32_optimizer
        amp_kwargs = {"init_scale": args['common']['fp16_init_scale']}
        if getattr(args['common'], "amp_scale_window", None) is not None:
            amp_kwargs["growth_interval"] = args['common']['amp_init_scale']
        self._grad_scaler = torch.cuda.amp.GradScaler(**amp_kwargs)
        self.min_loss_scale = args['common']['min_loss_scale']
    ...
    def setup_optimizer(cls, args, params, **kwargs):
        ...
        """
        fp32_optimizer = optim.setup_optimizer(args, params)
