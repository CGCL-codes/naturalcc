...
from ncc.data.summarization.graph_language_pair_dataset import GraphLanguagePairDataset
...
class GraphDataset(object):
    def __init__(self, dataset):
        ...
        self.sizes = np.asarray([len(graph) for graph in dataset])
...
@register_task('graph_summarization')
class GraphSummarizationTask(NccTask):
    """
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths):
        return GraphLanguagePairDataset(src_tokens, src_lengths, self.source_dictionary)

    def build_generator(self, models, args, extra_gen_cls_kwargs=None):
        from ncc.eval.summarization.tree2seq_generator import Tree2SeqGenerator
        return Tree2SeqGenerator(
            self.target_dictionary,
            beam_size=args['eval']['beam'],
            max_len_a=args['eval']['max_len_a'],
            max_len_b=args['eval']['max_len_b'],
            min_len=args['eval']['min_len'],
            normalize_scores=(not args['eval']['unnormalized']),
            len_penalty=args['eval']['lenpen'],
            unk_penalty=args['eval']['unkpen'],
            temperature=args['eval']['temperature'],
            match_source_len=args['eval']['match_source_len'],
            no_repeat_ngram_size=args['eval']['no_repeat_ngram_size'],
        )
