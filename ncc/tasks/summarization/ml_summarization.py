...
def load_langpair_dataset(
    ...
    def split_exists(split, src, data_path, domain):
        filename = os.path.join(data_path, domain, '{}.{}'.format(split, src))  # -{}.{} , tgt, lang
    ...
    for dm in domains:
        ...
        for k in itertools.count():
            ...
            if split_exists(split_k, src, data_path, dm):
                prefix = os.path.join(data_path, dm, '{}.'.format(split_k))  # {}-{}. , src, tgt
            elif split_exists(split_k, tgt, data_path, dm):
                prefix = os.path.join(data_path, dm, '{}.'.format(split_k))  # {}-{}. , tgt, src
...
@register_task('tl_summarization')
class TransferLearningSummarizationTask(NccTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            data_path, split, self.args['task']['domains'], src, self.src_dict, tgt, self.tgt_dict,
