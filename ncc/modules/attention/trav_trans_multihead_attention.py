...
class MultiheadAttention(nn.Module):
    ...
    See "Code Prediction by Feeding Trees to Transformers" for more details.
    """
    ...
    def __init__(
        ...
        """
        0: padding idx for upper triangular matrix
        1: self idx (U0D0) for diagonal
        >=2: UiDj (i,j>0) for lower triangular matrix
        """
        if self.maximum_relative_position is not None:
            self.relative_position_embed = nn.Embedding(self.maximum_relative_position + 1, self.num_heads,
                                                        padding_idx=0)
    ...
    def forward(
        ...
        if self.maximum_relative_position is not None:
            # for debug
            # rel_pos_indices = torch.randint(0, self.maximum_relative_position, size=(bsz, tgt_len, src_len)).long()
            rel_pos_emb = self.relative_position_embed(rel_pos_indices)
            rel_pos_emb = rel_pos_emb.permute(0, 3, 1, 2)  # [bsz, tgt, src, head] => [bsz, head, tgt, src]
            rel_pos_emb = torch.tril(rel_pos_emb)
            rel_pos_emb = rel_pos_emb.view(-1, tgt_len, src_len)
