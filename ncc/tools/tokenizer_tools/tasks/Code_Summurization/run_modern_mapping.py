...
class DataTrainingArguments:
    ...
    mapping_file: Optional[str] = field(
        default=None, metadata={"help": "Path to the mapping file for token ID correction."}
    )
    ### end of modified part
...
def main():
    ...
    # 加载并解析 mapping 文件
    mapping = None
    if data_args.mapping_file:
        logger.info(f"Loading token mapping from {data_args.mapping_file}")
        with open(data_args.mapping_file, 'r', encoding='utf-8') as f:
            mapping = json.load(f)
        logger.info("Token mapping loaded successfully.")
    ### end of modified part 
    ...
    # https://zhuanlan.zhihu.com/p/646852375  !!!!注意学习！！！
    ...
    def preprocess_function(examples, mapping=None):
        ...
        # inputs = examples['nl']
        # targets = examples['code']
        # inputs = examples['code'] # <--- 改成 code
        # targets = examples['docstring']   # <--- 改成 nl
        # inputs = [' '.join(js['code_tokens']) for js in examples]
        # 用 code_tokens 的好处
        # 标准化：token 顺序一致，空格和换行被统一处理
        # 模型训练友好：输入是可控的 token 序列，不会被奇怪字符打断
        # targets = [' '.join(js['docstring_tokens']) for js in examples] 写法错误！！后面再追究
        inputs = [' '.join(tokens) for tokens in examples['code_tokens']]
        targets = [' '.join(tokens) for tokens in examples['docstring_tokens']]
    ...
    tokenized_datasets = raw_datasets.map(
        ...
        ### modified part
        fn_kwargs={'mapping': mapping} # 将 mapping 字典传递给 preprocess_function
        ### end of modified part
    )
    ...
    # # ==================== 开始：DataCollator 输出调试代码 ====================
    # # 只有主进程才执行这个检查，避免在多GPU时重复打印
    # if training_args.local_rank in [-1, 0]:
    #     logger.info("--- Inspecting DataCollator Output ---")
        
    #     # 1. 从处理好的数据集中取出前3个样本
    #     num_samples_to_inspect = 2
    #     if len(train_dataset) < num_samples_to_inspect:
    #         logger.warning(f"Train dataset has fewer than {num_samples_to_inspect} samples. Inspecting all available samples.")
    #         num_samples_to_inspect = len(train_dataset)

    #     samples = [train_dataset[i] for i in range(num_samples_to_inspect)]
        
    #     # 2. 手动调用 data_collator 来打包这些样本
    #     # DataCollator 接收一个由字典组成的列表
    #     batch = data_collator(samples)

    #     print(f"\n[DataCollator] Batch created with {len(samples)} samples.")
    #     print("-" * 50)
    ...
    #     logger.info("--- Inspection Finished. Exiting script. ---")
        
    #     # 4. 打印完毕后直接退出脚本，不开始真正的训练
    #     sys.exit(0)
    # # ===================== 结束：DataCollator 输出调试代码 =====================
    ...
    def compute_metrics(eval_preds):
        ...
        ### modified part
        # 在解码前，对生成的 preds token ID 应用 mapping
        if mapping:
            # 需要逐个样本进行处理
            mapped_preds = []
            for pred_ids in preds.tolist(): # 将 numpy 数组转为 python 列表
                mapped_preds.append(modify_mapping(pred_ids, mapping))
            preds = np.array(mapped_preds) # 转回 numpy 数组以便后续处理
        ### end of modified part
        ...
        result = {"eval_bleu": bleu4} # <--- 只返回 BLEU 分数
        return result
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                ### modified part
                if mapping:
                    mapped_predictions_ids = []
                    for pred_ids in predictions_ids.tolist():
                        mapped_predictions_ids.append(modify_mapping(pred_ids, mapping))
                    predictions_ids = np.array(mapped_predictions_ids)
                ### end of modified part
