...
class Tree2SeqGenerator(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
    ...
    def generate(self, models, sample, **kwargs):
        ...
        bsz = len(encoder_input['src_roots'])
        max_len = min(
            self.max_len_b,
        ...
        # 1. encoder
        encoder_out = model.encoder(encoder_input['src_graphs'], encoder_input['src_roots'], encoder_input['src_nodes'],
                                    **kwargs)
        encoder_padding_mask = encoder_out['encoder_padding_mask']
        encoder_out = encoder_out['encoder_out']

        prev_output_tokens = encoder_out[0].new(bsz, 1).long().fill_(self.eos)
        ...
        for j in range(max_len):
            ...
            # now only support greedy sample
