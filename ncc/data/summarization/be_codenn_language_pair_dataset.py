...
def collate(
    ...
    if samples[0]['src_idx']:
        src_ids = [samples[i]['src_idx'] for i in sort_order.tolist()]
    else:
        src_ids = None
    if samples[0]['tgt_idx']:
        tgt_ids = [samples[i]['tgt_idx'] for i in sort_order.tolist()]
    else:
        tgt_ids = None
...
class BELanguagePairDataset(NccDataset):
    ...
    def __getitem__(self, index):
        ...
        # tgt_item = torch.cat((torch.Tensor([self.tgt_dict.pad()]).type_as(tgt_item), tgt_item))
        src_item = self.src[index]

        if self.src_ids:
            src_idx = self.src_ids[index]
        else:
            src_idx = None
        if self.tgt_ids:
            tgt_idx = self.tgt_ids[index]
        else:
            tgt_idx = None
