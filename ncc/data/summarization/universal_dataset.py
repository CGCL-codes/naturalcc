...
class UniversalDataset(NccDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.topk_idxs is not None and self.is_train:
            ...
            if self.expert_scores[self.dataset_ids[index]] is None:
                ...
                example['alpha'] = self.get_alpha(self.dataset_ids[index])
        return example
    ...
    def get_alpha(self, ds_id):
        if self.args['optimization']['alpha_strategy'] == 'fix':
            return self.DEFAULT_ALPHA
        if self.args['optimization']['alpha_strategy'] == 'threshold':
            if self['optimization']['student_scores'][ds_id] < self.expert_scores[ds_id] + 1:
                return self.DEFAULT_ALPHA
            else:
                return 0
        if self.args['optimization']['alpha_strategy'] == 'adaptive':
