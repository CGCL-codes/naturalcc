...
class Dictionary(object):
    ...
    #                       count_threshold: int = 5, add_unk: bool = True, add_pad: bool = False) -> 'Dictionary':
    def create_vocabulary(dict_file, max_size: int,
                          count_threshold: int = 5, add_unk: bool = True, add_pad: bool = False) -> 'Dictionary':
        tokens = Counter()
        with open(dict_file, 'r') as f:
            for line in f.readlines():
                tokens[' '.join(line.strip('\n').split(' ')[:-1])] = int(line.strip('\n').split(' ')[-1])

        if isinstance(tokens, Counter):
            token_counter = tokens
        else:
            token_counter = Counter(tokens)
        vocab = Dictionary(add_unk=add_unk, add_pad=add_pad)
