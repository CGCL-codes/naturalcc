...
class DenoisingDataset(NccDataset):
    """
    ...
    def __init__(
        ...
        self.mask_ratio = args['task']['mask']
        self.random_ratio = args['task']['mask_random']
        self.insert_ratio = args['task']['insert']
        self.rotate_ratio = args['task']['rotate']
        self.permute_sentence_ratio = args['task']['permute_sentences']
        ...
        self.replace_length = args['task']['replace_length']
        if not self.replace_length in [-1, 0, 1]:
            raise (f'invalid arg: replace_length={self.replace_length}')
        if not args['task']['mask_length'] in ['subword', 'word', 'span-poisson']:
            raise (f"invalid arg: mask-length={args['task']['mask_length']}")
        if args['task']['mask_length'] == 'subword' and not args['task']['replace_length'] in [0, 1]:
            raise (f'if using subwords, use replace-length=1 or 0')

        self.mask_span_distribution = None
        if args['task']['mask_length'] == 'span-poisson':
            _lambda = args['task']['poisson_lambda']
