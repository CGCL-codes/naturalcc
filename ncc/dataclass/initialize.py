...
def hydra_init(cfg_name="config") -> None:
    ...
    cs.store(name=f"{cfg_name}", node=NccConfig)

    for k in NccConfig.__dataclass_fields__:
        v = NccConfig.__dataclass_fields__[k].default
        try:
...
def add_defaults(cfg: DictConfig) -> None:
    ...
    from ncc.models import ARCH_MODEL_NAME_REGISTRY, MODEL_DATACLASS_REGISTRY
    from ncc.dataclass.utils import merge_with_parent
    from typing import Any

    OmegaConf.set_struct(cfg, False)

    for k, v in NccConfig.__dataclass_fields__.items():
        field_cfg = cfg.get(k)
