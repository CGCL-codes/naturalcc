...
def gen_parser_from_dataclass(
    ...
    dataclass_instance: NccDataclass,
    ...
    def get_kwargs_from_dc(
        dataclass_instance: NccDataclass, k: str
    ) -> Dict[str, Any]:
    ...
    for k in dataclass_instance._get_all_attributes():
        ...
        if field_name is None:
            continue
        elif inspect.isclass(field_type) and issubclass(field_type, NccDataclass):
...
def _override_attr(
    sub_node: str, data_class: Type[NccDataclass], args: Namespace
) -> List[str]:
    overrides = []

    if not inspect.isclass(data_class) or not issubclass(data_class, NccDataclass):
        return overrides
    ...
    for k, v in data_class.__dataclass_fields__.items():
        ...
        elif isinstance(val, NccDataclass):
...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in NccConfig.__dataclass_fields__.keys():
        overrides.extend(
            _override_attr(k, NccConfig.__dataclass_fields__[k].type, args)
        )

    if args is not None:
        if hasattr(args, "task"):
            from ncc.tasks import TASK_DATACLASS_REGISTRY
        ...
        if hasattr(args, "arch"):
            from ncc.models import ARCH_MODEL_REGISTRY, ARCH_MODEL_NAME_REGISTRY
...
def convert_namespace_to_omegaconf(args: Namespace) -> DictConfig:
    ...
    # configs will be in ncc/config after installation
    ...
    with omegaconf_no_object_check():
        ...
        if cfg.optimizer is None and getattr(args, "optimizer", None):
            ...
            from ncc.optim import OPTIMIZER_REGISTRY
        ...
        if cfg.lr_scheduler is None and getattr(args, "lr_scheduler", None):
            ...
            from ncc.optim.lr_scheduler import LR_SCHEDULER_REGISTRY
        ...
        if cfg.criterion is None and getattr(args, "criterion", None):
            ...
            from ncc.criterions import CRITERION_REGISTRY
...
def merge_with_parent(dc: NccDataclass, cfg: DictConfig, remove_missing=False):
    if remove_missing:
