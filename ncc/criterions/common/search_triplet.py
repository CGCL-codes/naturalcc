...
@register_criterion('search_triplet')
class SearchTripletCriterion(NccCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        def _forward(key):
            output, _ = model(**sample[key])
            output = output[sample[key]['src_tokens'] == self.task.target_dictionary.eos(), :]
            return output
