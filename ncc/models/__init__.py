...
def build_model(cfg: NccDataclass, task, from_checkpoint=False):
...
def register_model(name, dataclass=None):
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, BaseNccModel):
            raise ValueError(
                "Model ({}: {}) must extend BaseNccModel".format(name, cls.__name__)
            )
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="model", node=node, provider="ncc")
...
import_models(models_dir, "ncc.models")
