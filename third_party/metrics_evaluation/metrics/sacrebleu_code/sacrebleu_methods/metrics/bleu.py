...
_TOKENIZERS = {
    ...
    "code": "tokenizer_code.TokenizerCode",
}
...
def _get_tokenizer(name: str):
    ...
    return getattr(
        import_module(f".tokenizers.{module_name}", "metrics_evaluation.metrics.sacrebleu_code.sacrebleu_methods"),
        class_name,
    )
...
class BLEU(Metric):
    ...
    TOKENIZERS = ["none", "zh", "13a", "char", "intl", "ja-mecab", "spm", "code"]

    # mteval-v13a.pl tokenizer unless Chinese or Japanese is provided
    TOKENIZER_DEFAULT = "code"
    ...
    def __init__(
        ...
        # Sanity check
        assert (
            self.smooth_method in self.SMOOTH_DEFAULTS.keys()
        ...
        if tokenize is None:
            ...
            if self.trg_lang == "zh" and best_tokenizer != "zh":
                sacrelogger.warning("You should use the 'zh' tokenizer for Chinese.")
