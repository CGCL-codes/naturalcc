...
# Define the environment variable $SACREBLEU, or use the default of ~/.sacrebleu_methods.
#
...
SACREBLEU_DIR = os.environ.get(
    "SACREBLEU", os.path.join(USERHOME, ".sacrebleu_methods")
)
...
def _format_score_lines(
    scores: dict, width: int = 2, multiline: bool = True
) -> Dict[str, List[str]]:
...
def print_results_table(results: dict, signatures: dict, args: Namespace):
    ...
    if is_paired:
        test_type = (
            "bootstrap resampling" if args.paired_bs else "approximate randomization"
        )
        ...
        print(
            f"   generated by the same underlying process. For a given system and the {bline},"
        )
        print(
            "   the p-value is roughly the probability of the absolute score difference (delta)"
        )
        print(
            f"   or higher occurring due to chance, under the assumption that the {null_hyp.lower()} is correct."
        )
        print()
        print(
            f" - Assuming a significance threshold of 0.05, the {null_hyp.lower()} can be rejected"
        )
        print(
            '   for p-values < 0.05 (marked with "*"). This means that the delta is unlikely to be attributed'
        )
        print(
            f'   to chance, hence the system is significantly "different" than the {bline}.'
        )
        print(f"   Otherwise, the p-values are {pval_color}.")
        print()
        print(
            f' - NOTE: Significance does not tell whether a system is "better" than the {bline} but rather'
        )
        print(
...
def sanity_check_lengths(
    system: Sequence[str], refs: Sequence[Sequence[str]], test_set: Optional[str] = None
):
    n_hyps = len(system)
    if any(len(ref_stream) != n_hyps for ref_stream in refs):
        ...
        if test_set:
            sacrelogger.error(
                "This could be an issue with your system output "
            ...
            sacrelogger.error(
                "The test sets will be re-downloaded the next time you run sacreBLEU."
            )
        sys.exit(1)
...
def print_test_set(test_set, langpair, side, origlang=None, subset=None):
    ...
    if side == "src":
        ...
        files = [get_source_file(test_set, langpair)] + get_reference_files(
            test_set, langpair
        )
...
def get_files(test_set, langpair) -> List[str]:
    ...
    if num_refs == 1:
        ...
        reference_paths = [
            os.path.join(cachedir, f"{langpair}.{target}.{num}")
            for num in range(num_refs)
        ]
...
def check_md5sum(dest_path, expected_md5):
    ...
    if expected_md5 is not None:
        ...
        if cur_md5 != expected_md5:
            sacrelogger.error(
                f"Fatal: MD5 sum of downloaded file was incorrect (got {cur_md5}, expected {expected_md5})."
            )
            sacrelogger.error(
                f"Please manually delete {tarball!r} and rerun the command."
            )
            sacrelogger.error(
...
def get_langpairs_for_testset(testset: str) -> List[str]:
    ...
    return list(
        filter(lambda x: re.match(r"\w\w\-\w\w", x), DATASETS.get(testset, {}).keys())
    )
...
def filter_subset(systems, test_sets, langpair, origlang, subset=None):
    ...
    for test_set in test_sets.split(","):
        ...
        if not rawfile.endswith(".sgm"):
            raise Exception(
                f"--origlang and --subset supports only *.sgm files, not {rawfile!r}"
            )
...
def print_subset_results(metrics, full_system, full_refs, args):
    ...
    origlangs = (
        args.origlang
        if args.origlang
        else get_available_origlangs(args.test_set, args.langpair)
    )
    ...
    for key, scores in results.items():
        ...
        for n_system, score in scores:
            print(
                f"{key}: sentences={n_system:<6} {score.name:<{max_metric_width}} = {score.score:.{w}f}"
            )
