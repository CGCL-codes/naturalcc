...
parser.add_argument("--dataset", type=str, default='codetrans', help="dataset")
...
if __name__ == '__main__':
    ...
    train_dataset = CrossPairDataset(config, vocab, data_path=DATA_PATH, mode="train", src_lang=SRC_LANG,
                                     tgt_lang=TGT_LANG)
    ...
    valid_dataset = CrossPairDataset(config, vocab, data_path=DATA_PATH, mode="valid",
        ...
        def train():
            ...
            for batch_idx, batch in enumerate(train_dataloader, start=1):
                ...
                with torch.cuda.amp.autocast(enabled=True):
                    src_tokens, src_masks, src_positions, attn_mask, tgt_tokens, tgt_masks, _ = batch
                    logits = model.forward(src_tokens, src_masks, src_positions, attn_mask, tgt_tokens, tgt_masks)
                    ...
                    for batch in valid_dataloader:
                        ...
                        with torch.cuda.amp.autocast(enabled=True):
                            ...
                            logits = model.forward(
                                src_tokens, src_masks, src_positions, attn_mask, tgt_tokens, tgt_masks
                            )
                            batch_loss = CrossEntropyLoss(logits, tgt_tokens, mask=tgt_masks)
                        if DEVICE_NUM > 1:
                            batch_loss = batch_loss.mean()
                ...
                with torch.no_grad():
                    ...
                    for batch in valid_dataloader:
                        ...
                        if DEVICE_NUM > 0:
                            batch = move_to_cuda(batch)
                        with torch.cuda.amp.autocast(enabled=True):
                            # src_tokens, src_masks, src_positions, attn_mask, tgt_tokens, tgt_masks, indices = batch
                            src_tokens, src_masks, src_positions, attn_mask, _, _, indices = batch

                            if DEVICE_NUM > 1:
                                encoder_output = model.module.encoder_forward(src_tokens, src_masks, src_positions,
                                                                              attn_mask)
                                preds = model.module.greedy_decode(encoder_output[0], src_masks)
                            else:
                                encoder_output = model.encoder_forward(src_tokens, src_masks, src_positions, attn_mask)
                                preds = model.greedy_decode(encoder_output[0], src_masks)
                            # preds = model.beam_decode(encoder_outputs[0], src_masks)
                    ...
                    references = [[code.split()] for code in valid_dataset.tgt_data['code']]
