...
def single_main(args, init_distributed=False):
    ...
    if PathManager.exists(args['checkpoint']['init_checkpoint']):
        ...
        for module_name, module_param in pretrained_params.items():
            if module_name in init_params:
                if init_params[module_name].data.size() == module_param.data.size():
                    ...
                    # emebedding
                    # [50004, ?] <- [50005, ?], and the last is [mask]
    ...
    while (
        ...
        # validate(args, trainer, task, epoch_itr, args['dataset']['gen_subset'].split(','))
        # train for one epoch
...
def cli_main():
    ...
    parser.add_argument(
        ...
        # default='config/codetrans/base/java-csharp'
        default='config/codetrans/base/csharp-java'
        # default="config/codetrans/vartrans/java-csharp"
    )
