...
from run.translation.codebert import config
...
parser.add_argument("--SRC_LANG", default='python', type=str, help="source language")
...
parser.add_argument("--dataset", default='avatar', type=str, help="dataset")
parser.add_argument(
...
if __name__ == '__main__':
    ...
    test_dataset = CrossPairDataset(
        vocab, data_path=DATA_PATH, mode="test", src_lang=SRC_LANG, tgt_lang=TGT_LANG,
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader):
            ...
            src_tokens, src_masks, _, _, indices = batch
            encoder_output = model.encoder_forward(src_tokens, src_masks)
            preds = model.beam_decode(encoder_output, src_masks, max_length=500)
        ...
        references = [[c.split() for c in code] if isinstance(code, list) else [code.split()]
                      for code in test_dataset.tgt_code[:len(predictions)]]
        ...
        references = {idx: code if isinstance(code, list) else [code]
                      for idx, code in enumerate(test_dataset.tgt_code[:len(predictions)])}
