...
def train(args, trainer, task, epoch_itr):
    ...
    stats['accuracy'] = f'{round(100. * match / total, 2)}%'
...
def validate(args, trainer, task, epoch_itr, valid_subsets, dev_subsets, dev_refs):
    ...
    for subset in valid_subsets:
        ...
        progress.print({
            'accuracy': f'{round(100. * valid_acc, 2)}%', 'bleu': stats['bleu'], 'loss': stats['loss'],
        }, tag=subset, step=trainer.get_num_updates())
...
def get_dev_stats(args, trainer, stats):
    if 'nll_loss' in stats and 'ppl' not in stats:
        stats['ppl'] = utils.get_perplexity(stats['nll_loss'])
    stats['num_updates'] = trainer.get_num_updates()
...
def load_refs(ref_prefix):
    ref_id_file = ref_prefix + '.id'
    ref_docstring_tokens_file = ref_prefix + '.docstring_tokens'
    refs = {}
    with open(ref_id_file, 'r') as id_reader, open(ref_docstring_tokens_file, 'r') as doc_reader:
        for id, tokens in zip(id_reader, doc_reader):
            id, tokens = int(ujson.loads(id)), ujson.loads(tokens)
            if id in refs:
                refs[id].append(tokens)
            else:
                refs[id] = [tokens]
    return refs
...
def single_main(args, init_distributed=False):
    ...
    while (
        ...
        if not args['dataset']['disable_validation'] and epoch_itr.epoch % args['dataset']['validate_interval'] == 0:
            valid_acc, dev_prf = validate(args, trainer, task, epoch_itr, valid_subsets, dev_subsets, dev_refs=None)
        else:
        ...
        if epoch_itr.epoch % args['checkpoint']['save_interval'] == 0:
            checkpoint_utils.save_checkpoint(args, trainer, epoch_itr, valid_acc)
