...
def test_chrf(hypotheses, references, expected_score):
    score = sacrebleu.corpus_chrf(
        hypotheses, [references], char_order=6, word_order=0, beta=3, eps_smoothing=True
    ).score
    assert abs(score - expected_score) < EPSILON


@pytest.mark.parametrize(
    "hypotheses, references, expected_score", test_cases_effective_order
)
...
@pytest.mark.parametrize(
    "hypotheses, references, expected_score", test_cases_keep_whitespace
)
...
@pytest.mark.parametrize(
    "hypothesis, references, expected_score", test_sentence_level_chrf
)
