...
import preprocess.typilus.make_dataset
...
def make_dataset(from_type, to_type,
    ...
    if(to_type == "typilus"):
        if(from_type == "raw"):
            from preprocess.typilus.make_dataset import from_raw
            if(to_path == None):
                to_path = TYPILUS_GRAPH_DIR
            preprocess.typilus.make_dataset.from_raw(from_path, to_path)
...
def prepare_dataset(ds: str, **kwargs):
    if (ds == "codesearchnet"):
        from preprocess import codesearchnet
        codesearchnet.dataset_download.download()
        codesearchnet.attributes_cast.attributes_cast()
    elif (ds == 'python_wan'):
        from preprocess import python_wan
        python_wan.dataset_download.download()
    elif (ds == "typilus"):
        from preprocess.typilus import dataset_migration, flatten
...
def binarize_dataset(ds: str):
    if (ds == "typilus"):
        from preprocess.typilus.preprocess import preprocess
