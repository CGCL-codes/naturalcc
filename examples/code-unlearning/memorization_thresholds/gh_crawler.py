...
MIN_STARS = 500
CREATED = '2023-06-01'
LANGUAGE = "python" if len(sys.argv) <= 1 else sys.argv[1]  # Default to Python, if none passed.
...
def main():
	...
	with open(f'TopLists/{LANGUAGE}-top-repos.txt', 'a+') as f:
...
def run_query(max_stars):
	...
	while end_cursor != "":
		...
		query = f"""
		{{
		  search(query: "language:{LANGUAGE} fork:false created:>{CREATED} sort:stars stars:<{max_stars}", type: REPOSITORY, first: 100 {', after: "' + end_cursor + '"' if end_cursor else ''}) {{
			edges {{
			  node {{
				... on Repository {{
				  url
