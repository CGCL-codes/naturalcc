...
plugins = [
    {"name": "ArtifactoryDetector"},  # Scans for Artifactory credentials.
    {"name": "AWSKeyDetector"},  # Scans for AWS keys.
    {"name": "AzureStorageKeyDetector"},  # Scans for Azure Storage Account access keys.
    {"name": "CloudantDetector"},  # Scans for Cloudant credentials.
    {"name": "DiscordBotTokenDetector"},  # Scans for Discord Bot token.
    {"name": "GitHubTokenDetector"},  # Scans for GitHub tokens.
    {"name": "GitLabTokenDetector"},  # Scans for GitLab tokens.
    {"name": "IbmCloudIamDetector"},  # Scans for IBM Cloud IAM Key.
    {"name": "IbmCosHmacDetector"},  # Scans for IBM Cloud Object Storage HMAC credentials.
    {"name": "JwtTokenDetector"},  # Scans for JWTs.
    {"name": "MailchimpDetector"},  # Scans for Mailchimp keys.
    {"name": "NpmDetector"},  # Scans for NPM tokens.
    {"name": "OpenAIDetector"},  # Scans for OpenAI tokens.
    {"name": "PypiTokenDetector"},  # Scans for PyPI tokens.
    {"name": "SendGridDetector"},  # Scans for SendGrid API keys.
    {"name": "SlackDetector"},  # Scans for Slack tokens.
    {"name": "SoftlayerDetector"},  # Scans for Softlayer credentials.
    {"name": "SquareOAuthDetector"},  # Scans for Square OAuth Secrets.
    {"name": "StripeDetector"},  # Scans for Stripe keys.
    {"name": "TelegramBotTokenDetector"},  # Scans for Telegram bot tokens.
    {"name": "TwilioKeyDetector"},  # Scans for Twilio API keys.
    
    # the entropy detectors esp Base64 need the gibberish detector on top
    {"name": "Base64HighEntropyString"},  # Scans for random-looking base64 encoded strings.
    {"name": "HexHighEntropyString"},  # Scans for random-looking hex encoded strings.
    
    # Scans for public ip address (ipv4). Some non-public ipv4 addresses are ignored.
...
def is_hash(content, value):
    ...
    try:
        ...
        lines = content[:res].splitlines()
        target_line = lines[-1]
    # except (ValueError, IndexError):
    except Exception as e:
        print(f"Value not found in content, value: {value}, content: {content}, index: {content.index(value)}.")
        return False
...
def detect_keys(content, suffix=".txt"):
    ...
    if secrets_set:
        for secret in secrets_set[0]:
            secret.secret_value = secret.secret_value.strip("\\")
