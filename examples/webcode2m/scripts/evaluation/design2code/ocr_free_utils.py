...
def get_blocks_from_image_diff_pixels(origin_image_path, image_path, html_text_color_tree, different_pixels):
    ...
    for item in html_text_color_tree:
        ...
        color = average_color(origin_image_path, coords)

        blocks.append({'text': item[0].lower(), 'bbox': (y_min / y_w, x_min / x_w, (y_max - y_min + 1) / y_w, (x_max - x_min + 1) / x_w), 'color': color})
    return blocks


def get_itermediate_names(html_path, tmp_dir):
    name = Path(html_path).name.split('.')[0]
    out_dir = Path(tmp_dir)
    return  out_dir / f"{name}_p.html", out_dir / f"{name}_p_1.html", out_dir / f"{name}_p.png", out_dir / f"{name}_p_1.png"

def get_blocks_ocr_free(html_path, image_path,tmp_dir):
    p_html, p_html_1, p_png, p_png_1 = get_itermediate_names(html_path, tmp_dir)
    process_html(html_path, p_html)
    process_html(html_path, p_html_1, offset=50)

    os.system(f"python3 {Path(__file__).parent.parent}/html2screenshot.py --input {p_html} --output {p_png}")
    os.system(f"python3 {Path(__file__).parent.parent}/html2screenshot.py --input {p_html_1} --output {p_png_1}")
    ...
    if different_pixels is None:
        ...
        os.system(f"rm {str(p_html)} {str(p_png)} {str(p_html_1)} {str(p_png_1)}")
        return []

    html_text_color_tree = flatten_tree(extract_text_with_color(p_html))
    try:
        blocks = get_blocks_from_image_diff_pixels(str(image_path), str(p_png), html_text_color_tree, different_pixels)
    except:
