...
def choose_dataset_class(args):
  ...
  """
  if args['model']['model_type'] == 'CodeBert-disk' or args['model']['model_type']=='GraphCodeBert-disk':
    dataset_class = data.CodeBertDataset
  else:
...
def choose_probe_class(args):
  ...
  Args:
    args: the global config dictionary built by yaml.t
  Returns:
...
def execute_experiment(args, train_probe, report_results):
  ...
  """
  dataset_class = choose_dataset_class(args)  #构造数据集
  task_class, reporter_class, loss_class = choose_task_classes(args)
  probe_class = choose_probe_class(args) # Constructing
