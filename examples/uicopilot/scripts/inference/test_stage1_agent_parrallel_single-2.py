...
num_max_infer = 8
model_name1 = 'vu-agent-4v-AgentI2C2'
model_name2 = 'vu-agent-4v-AgentI2C2-opt'
...
def infer_bbox(image, model_bbox, processor, device):
    model_bbox.eval()
    with torch.no_grad():
        input = f'<body bbox=['
...
def gen(image,  model_bbox,ind, processor, device,output_dir1,output_dir2,max_depth=100, min_area=100):
    try:
        imgs = []
        agent_i2c = AgentI2C2()
        ...
        prediction_html = infer_bbox(image, model_bbox, processor, device)
        ...
        with open(f'{output_dir1}/bbox_leaf_nodes.json', 'w') as f:
            json.dump(indexList, f, indent=4)
        with open(f'{output_dir2}/bbox_leaf_nodes.json', 'w') as f:
            json.dump(indexList, f, indent=4)
        return html,html2,imgs
    except Exception as e:
            tqdm.write(f"Attempt  failed for index {ind}: {e}")
            sys.stdout.flush()
                # 返回一个默认的 HTML 作为异常处理
...
def gen_process(indices, device_index):
    ...
    for idx in indices:
        ...
        output_dir1=os.path.join(save_path1,f'{md5}')
        os.makedirs(output_dir1,exist_ok=True)
        output_dir2=os.path.join(save_path2,f'{md5}')
        os.makedirs(output_dir2,exist_ok=True)


        t_start = time.time()
        html, html2, imgs = gen(image, model_bbox, idx, processor, device,output_dir1,output_dir2)
        ...
        while attempt < max_attempts:
            if 'Failed' not in html and 'Failed' not in html2 and isinstance(html, str) and isinstance(html2, str):
                ...
                attempt += 1
                tqdm.write(f"Attempt {attempt} failed to generate HTML for index {idx}: {md5}")
                sys.stdout.flush()
                if attempt < max_attempts:  # 只有在未达到最大尝试次数时才重新请求
                    html, html2, imgs = gen(image, model_bbox, idx, processor, device,output_dir1,output_dir2)
                else:
                    tqdm.write(f"Failed after {max_attempts} attempts for index {idx}: {md5}")
                    sys.stdout.flush()
        sys.stdout.flush()

# 修改并行部分以适应离散索引
indices_to_process =[417,439, 462,487, 490,  492,498, 511]




# 修改为并行处理的函数
with Pool(num_max_infer) as p:
    p.starmap(gen_process, [(indices_to_process[i::num_max_infer], i) for i in range(num_max_infer)])
