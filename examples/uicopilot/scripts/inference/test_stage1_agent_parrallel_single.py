...
num_max_infer = 8
model_name1 = 'a-30-d-6'
model_name2 = 'a-30-d-6-opt'
test_data_name = 'vision2ui'
result_path1 = f'/data02/projects/vision2ui/results/ablation-4v/{model_name1}-{test_data_name}'
result_path2 = f'/data02/projects/vision2ui/results/ablation-4v/{model_name2}-{test_data_name}'
...
def gen(image,  model_bbox,ind, processor, device, max_depth=100, min_area=100):
    try:
        imgs = []
        agent_i2c = AgentI2C()
...
def gen_process(indices, device_index):
    ...
    for idx in indices:
        ...
        min_area = image_area * 0.3  # 设置 min_area 为图像面积的 30%
        ...
        t_start = time.time()
        html, html2, imgs = gen(image, model_bbox, idx, processor, device,max_depth=6,min_area=min_area)
