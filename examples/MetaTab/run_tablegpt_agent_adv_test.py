    ...
    for table_idx, d in enumerate(data):
        ...
        if perturbation == "none":
           ...
           table = construct_markdown_table(**d["row_column_shuffled_table"])
            ...
            if d["anonymized_tables"]:
                ...
                continue
        elif perturbation == "key_value":
            table = construct_markdown_table(**d["key_value_table"])
        ...
        for idx in index_list:
           ...
           #     continue
            ...
            import numpy as np
            import pandas as pd
            # 设置pandas显示选项，确保所有内容都显示
            pd.set_option('display.max_rows', None)  # 显示所有行
            pd.set_option('display.max_columns', None)  # 显示所有列
            pd.set_option('display.width', None)  # 自动调整宽度
            pd.set_option('display.max_colwidth', None)  # 显示完整的列内容
            def remove_irrelevant_cell(question, df):
                """
                删除DataFrame中与问题答案无关的单元格

                参数:
                question: str, 用户提出的问题
                df: pandas DataFrame, 需要处理的表格数据

                返回:
                处理后的DataFrame，其中一个单元格已被设置为NaN
                """
                # 这里使用简单的启发式方法：
                # 1. 首先尝试理解问题可能关注的列
                # 2. 然后找到最不相关的单元格进行删除
                ...
                for col in df.columns:
                    if isinstance(col, str) and col.lower() in question_lower:
                        relevant_columns.append(col)

                # 如果没有直接匹配的列，尝试基于常见关键词推断
                    ...
                    for col in df.columns:
                        if isinstance(col, str):
                            ...
                            for keyword in question_keywords:
                                if keyword in col_lower and len(keyword) > 3:  # 避免短词误匹配
                                    relevant_columns.append(col)
                                    break

                # 如果还是找不到相关列，随机选择一列作为"相关列"
                if not relevant_columns:
                    relevant_columns = [df.columns[0]]

                # 找到最不相关的行（与问题相关性最低的行）
                # 这里使用简单的策略：选择数值列中值最小的行，或者第一行
                irrelevant_row_idx = 0
                for col in df.select_dtypes(include=[np.number]).columns:
                    if col not in relevant_columns:
                        min_val_idx = df[col].idxmin()
                        if pd.notna(min_val_idx):
                            irrelevant_row_idx = min_val_idx
                            break
                ...
                # 如果所有列都被认为是相关的，选择最后一列
                if irrelevant_col is None:
                    irrelevant_col = df.columns[-1]

                # 创建DataFrame的副本以避免修改原始数据
                result_df = df.copy()

                # 将选定的单元格设置为NaN（删除）
                result_df.at[irrelevant_row_idx, irrelevant_col] = " "

                return result_df


            df = remove_irrelevant_cell(question, df)
