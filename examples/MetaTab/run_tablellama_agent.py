...
class Config:
    def __init__(self):
        self.base_model = "./tablellama"  # Smaller model for demo
        self.cache_dir = "./cache"
        self.temperature = 0.6
        self.top_p = 0.9
        self.max_gen_len = 256  # Shorter for demo
        ...
        for idx in index_list:
           ...
           #     continue
            ...
            for _ in range(self_consistency):
                ...
                texts.append(text)
                """
                from prompt.wtq.agent import agent_prefix, agent_prefix_with_omitted_rows_guideline, table_llama_agent_prefix
                # Prepare the prompt (replacing TableAgent initialization)
                if use_full_table:
                    table = df.to_markdown()
                    prompt = table_llama_agent_prefix.replace("[TABLE]", table)
                else:
                ...
                for i in range(5):  # You'll need to define max_depth somewhere
                    ...
                    text = out  # The generated response becomes our text
