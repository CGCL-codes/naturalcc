...
def main(
    ...
    # load model
    llm = LLM(model="./tableLLM", tensor_parallel_size=1, max_model_len=4096)
        ...
        for idx in index_list:
           ...
           #     continue
            ...
            for _ in range(self_consistency):
                ...
                texts.append(text)
                """
                from prompt.wtq.agent import agent_prefix, agent_prefix_with_omitted_rows_guideline, table_LLM_agent_prefix2
                # Prepare the prompt (replacing TableAgent initialization)
                if use_full_table:
                    table = df.to_markdown()
                    prompt = table_LLM_agent_prefix2.replace("[TABLE]", table)
                else:
                ...
                for i in range(5):  # You'll need to define max_depth somewhere
                    ...
                    #print(responses[0].outputs[0].text.lstrip(' '))
                    #exit(1)
                    ...
                    def remove_pd_read_lines(code, max_remove=2):
                        # 匹配 df = pd.read_xxx(...) 或直接 pd.read_xxx(...)
                        pattern = r'^\s*(.*\s*=\s*)?pd\.read\w+\s*\(.*\)\s*(#.*)?$'
                        lines = code.split('\n')
                        new_lines = []
                        removed = 0

                        for line in lines:
                            if removed < max_remove and re.fullmatch(pattern, line.strip()):
                                removed += 1
                                continue  # 跳过这行（相当于删除）
                        ...
                        return '\n'.join(new_lines)
                    """
                    def remove_output_lines(text):

                        # 使用正则表达式匹配包含output的行（忽略大小写）
                        pattern = r'^.*output.*$'
                        # 使用re.MULTILINE模式让^和$匹配每行的开头结尾
                        # 使用re.IGNORECASE忽略大小写
                        return re.sub(pattern, '', text, flags=re.MULTILINE | re.IGNORECASE)
                    code = remove_pd_read_lines(code)
                    code = remove_output_lines(code)
                    """
                    code = remove_pd_read_lines(code)
                    #print("code:")
                    #print(code)
                    ...
                    try:
                        @timeout_decorator.timeout(60)  # 5秒超时
                        def run_code():
                        ...
                        observation, memory = run_code()
                    except timeout_decorator.TimeoutError:
                        continue
                        observation, memory = "Timeout: Code took too long", memory
                    except Exception as e:
                        continue
                        observation, memory = f"Error: {e}", memory
