...
def main(
    ...
    for table_idx, d in enumerate(data):
        ...
        def split_dataframe_safely(df):
            """
            Safely split dataframe into two parts considering:
            - Odd/even row counts
            - Header row presence
            ...
            # Special case: Single row with header
            if has_header and len(df) == 1:
                return df.copy(), pd.DataFrame(columns=df.columns)

            split_point = (len(df) + int(has_header)) // 2

            if has_header:
                part1 = df.iloc[:split_point].copy()
                part2 = df.iloc[split_point:].copy()
            else:
                part1 = df.iloc[:split_point].copy()
                part2 = df.iloc[split_point:].copy()
