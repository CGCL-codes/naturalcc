...
def transform_markdown_table(
        md_table: str,
        cat_col: int = 0,  # 默认第一列为类别列
        auto_detect: bool = False  # 是否自动检测类别列
) -> str:
    """
    转换Markdown表格格式，将指定类别列转换为分组显示

    :param md_table: 原始Markdown表格字符串
    :param cat_col: 类别列的索引（从0开始）
    :param auto_detect: 是否自动检测类别列（会覆盖cat_col参数）
    :return: 转换后的Markdown表格
    """
    # 解析原始表格
    lines = [line.strip() for line in md_table.split('\n') if line.strip()]
    if len(lines) < 2:
        return md_table  # 不是有效表格

    # 解析表头
    headers = [h.strip() for h in lines[0][1:-1].split('|')]
    num_cols = len(headers)

    # 自动检测类别列
    if auto_detect:
        # 这里可以调用之前实现的自动检测函数
        # 为简化示例，假设检测逻辑是：选择重复值最多的非数值列
        data_rows = []
        for line in lines[2:]:
        ...
        if data_rows:
            # 简单检测逻辑（实际项目建议用前面完整的检测函数）
            col_stats = []
            for i in range(num_cols):
                values = [row[i] for row in data_rows]
                try:
                    # 尝试转换为数字，如果不能转则是类别列候选
                    [float(v) for v in values if v]
                    is_numeric = True
                except ValueError:
                    is_numeric = False
                uniqueness = len(set(values)) / len(values)
                col_stats.append((i, is_numeric, uniqueness))

            # 选择非数值且唯一性最低的列
            candidates = [x for x in col_stats if not x[1]]
            if candidates:
                cat_col = min(candidates, key=lambda x: x[2])[0]

    # 确保类别列索引有效
    cat_col = max(0, min(cat_col, num_cols - 1))

    # 构建新表头（排除类别列）
    new_headers = [h for i, h in enumerate(headers) if i != cat_col]
    new_table = [
        f"| {' | '.join(new_headers)} |",
        f"|{'|'.join(['---'] * len(new_headers))}|"
    ]
    ...
    for line in lines[2:]:
        ...
        category = cells[cat_col]
        data_values = [v for i, v in enumerate(cells) if i != cat_col]

        # 新类别时添加标题行
        if category != current_category:
            new_table.append(
                f"| {category} |{'|'.join([''] * (len(new_headers) - 1))}|"
            )
            current_category = category

        # 添加数据行
        new_table.append(f"| {' | '.join(data_values)} |")

    return '\n'.join(new_table)


def identify_category_column_smart(df: pd.DataFrame) -> int:
    """
    智能识别类别列（优化版）
    返回最适合作为类别列的索引
    """
    ...
    # 策略1：优先检查列名含有关键词的列
    category_keywords = {'category', 'type', 'class', 'kind', '类别', '类型', '种类'}
    for i, col in enumerate(df.columns):
        if any(kw in str(col).lower() for kw in category_keywords):
            return i

    # 策略2：选择唯一值最少且非数值型的列，且该列不能含有任何NaN
    # 1. 找出所有不含NaN的列
    non_nan_cols = df.columns[df.notna().all()]

    if len(non_nan_cols) == 0:
        raise ValueError("所有列都含有NaN，无法选择候选列")

    # 2. 在这些列中计算nunique比例并排序
    nunique_ratios = (df[non_nan_cols].nunique() / len(df)).sort_values()

    # 3. 遍历，找到第一个非数值型的列
    for candidate in nunique_ratios.index:
        if not pd.api.types.is_numeric_dtype(df[candidate]):
            return df.columns.get_loc(candidate)

    # 如果所有列都是数值型，默认返回第一个（或抛出异常）
    return df.columns.get_loc(non_nan_cols[0])
           ...
           #     continue
            ...
            for _ in range(self_consistency):
                ...
                if use_full_table:
                    table = df.to_markdown()
                    try:
                        cat_col = identify_category_column_smart(df)
                    except:
                        break

                    table = transform_markdown_table(table, cat_col)
