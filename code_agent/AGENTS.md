...
- `agent_ui.py`
  Gradio 页面入口。新版界面采用“紧凑顶部抬头 + 左右双卡片”布局：左侧负责项目与模型配置，右侧负责任务与执行。主卡片默认控制在首屏内，桌面端会按浏览器宽高自适应收缩，超出内容在各自卡片内部滚动。支持项目目录选择、目标文件多选、模型/API Key 输入、prompt 预览、Aider 流式运行，以及命令输出与状态分面板展示。

- `agent_ui_assets.py`
  `agent_ui.py` 的静态资源文件。集中放顶部抬头 HTML、自定义 CSS 和操作说明文案，后续想改视觉样式、版心高度、隐藏/显示样式或静态说明时优先看这里。

- `agent_ui_bindings.py`
  `agent_ui.py` 的事件绑定文件。集中管理按钮和输入框的 `.click()` / `.change()` 注册逻辑，后续想改某个按钮怎么联动、切哪个面板、刷新哪些组件时优先看这里。
...
- `agent_ui.py`
  纯 UI 壳层。新版界面分成两个主要区域：
  - 左侧卡片有两个按钮面板：`常规配置`、`工程概览 / 进阶控制`
  - 右侧卡片有三个按钮面板：`开发指令`、`命令行生成内容`、`操作说明 / 状态`
  它会扫描项目文件并忽略 `.git`、`__pycache__`、`node_modules`、`datasets`、`.log`、`.ps` 等噪音内容，同时动态显示配置校验、项目概览、等效 CLI 命令和任务状态；左右主卡片内部带独立滚动区，避免整页过长，并在较窄窗口下自动切成单列。底部按钮导航固定在卡片外层底部，正文区域单独滚动，避免内容从按钮下方透出；预览或执行后右侧会自动切到“命令行生成内容”面板。当前控制台面板内部是“左侧日志输出 + 右侧等效 CLI”的双栏结构，且滚动条默认隐藏、交互时再显现。静态 CSS / 抬头 / 操作说明已拆到 `agent_ui_assets.py`，事件绑定已拆到 `agent_ui_bindings.py`，减少了 UI 主文件长度。
...
- 想改 UI 行为、文件筛选、页面交互：先看 `agent_ui.py`
- 想改 UI 样式、抬头 HTML、首屏高度、隐藏面板样式：看 `agent_ui_assets.py`
- 想改按钮绑定、视图切换联动、输入变化后的刷新逻辑：看 `agent_ui_bindings.py`
...
默认会启动 Gradio，本地端口是 `7860`。启动后：

- 顶部显示软件抬头与说明
- 左侧卡片用于项目根目录、目标文件、API Key、模型和进阶控制
- 右侧卡片用于输入开发指令、查看命令行生成内容以及查看操作说明 / 实时状态
- 点击 `预览 Prompt` 或 `生成并执行` 后，界面会自动切换到“命令行生成内容”面板
- 桌面端会优先维持横向双栏并在卡片内部滚动，窗口收窄后自动切成单列
