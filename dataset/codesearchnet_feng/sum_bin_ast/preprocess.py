...
from ncc.utils.graph_utils import (tree2dgl)


def make_dgl_binary_dataset(vocab, input_file, output_file):
    graph_batch, ids = [], []
...
def main(args):
    ...
    if args['preprocess']['srcdict']:
        ...
        src_dict = task.build_dictionary(
            filenames,
            tokenize_func=tokenization.bin_ast_tokenizer,
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        if args['preprocess']['dataset_impl'] == "raw":
            ...
            if lang == 'docstring_tokens':
                make_binary_dataset(vocab, in_file, out_file, num_workers)
            else:
                make_dgl_binary_dataset(vocab, in_file, out_file)
