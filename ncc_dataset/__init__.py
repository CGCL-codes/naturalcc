...
import ncc_dataset.typilus.make_dataset
...
def make_dataset(from_type, to_type,
    ...
    if(to_type == "typilus"):
        if(from_type == "raw"):
            from ncc_dataset.typilus.make_dataset import from_raw
            if(to_path == None):
                to_path = TYPILUS_GRAPH_DIR
            ncc_dataset.typilus.make_dataset.from_raw(from_path, to_path)
...
def binarize_dataset(ds: str):
    if (ds == "typilus"):
        from ncc_dataset.typilus.preprocess import preprocess
