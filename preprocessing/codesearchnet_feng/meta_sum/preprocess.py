...
def dump_proj(proj_file: str, proj_idx_file: str):
    with open(proj_file, 'r') as reader, open(proj_idx_file, 'w') as writer:
        proj_idx = OrderedDict({})
        for _idx, _proj in enumerate(reader):
            _proj = ujson.loads(_proj)
            if _proj in proj_idx:
                proj_idx[_proj].append(_idx)
            else:
                proj_idx[_proj] = [_idx]
        print(ujson.dumps(proj_idx), file=writer)
...
def main(args):
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        ...
        proj_file = in_file[:in_file.rfind('.')] + '.repo'
        proj_idx_file = out_file[:out_file.rfind('.')] + '.proj'
        os.makedirs(os.path.dirname(proj_idx_file), exist_ok=True)
        dump_proj(proj_file=proj_file, proj_idx_file=proj_idx_file)
