...
from preprocessing.ast_parser.tree_sitter import TreeSitterASTParser
...
class AttrFns:
    ...
    def ast_fn(filename, dest_filename, idx, start=0, end=-1, *args):
        ...
        parser = TreeSitterASTParser(SO_FILE=os.path.join(kwargs['so_dir'], f"{kwargs['lang']}.so"),
...
if __name__ == '__main__':
    ...
    for lang, mode in itertools.product(args.languages, MODES):
        for tgt_attr in args.attrs:
            ...
            src_filename = os.path.join(args.attributes_dir, f"top{args.topk}", lang, f"{mode}.{src_attr}")
            if PathManager.exists(src_filename):
                tgt_filename = \
                    os.path.join(args.attributes_dir, f"top{args.topk}", lang, f"{mode}.{tgt_attr}")
