...
def binarize(args: Dict, filename: str, dict: Dictionary, in_file: str,
    ...
    tokenize = tokenizer_funcs.list_tokenizer if str.endswith(filename, 'docstring_tokens') else ujson.loads
    res = Binarizer.binarize(filename, dict, consumer, tokenize=tokenize, append_eos=append_eos, offset=offset, end=end)
...
def main(args):
    ...
    if args['preprocess']['srcdict']:
        ...
        src_dict = task.build_dictionary(
            filenames,
            tokenize_func=tokenizer_funcs.sbt_tokenizer,
    ...
    if target:
        if args['preprocess']['tgtdict']:
            ...
            tgt_dict = task.build_dictionary(
                filenames,
                tokenize_func=tokenizer_funcs.list_tokenizer,
                workers=args['preprocess']['workers'],
                threshold=args['preprocess']['thresholdsrc'],
    ...
    def make_binary_dataset(vocab: Dictionary, input_file, output_file, num_workers: int):
        ...
        tokenize = tokenizer_funcs.list_tokenizer if str.endswith(input_file, 'docstring_tokens') else ujson.loads
        merge_result(
            Binarizer.binarize(
