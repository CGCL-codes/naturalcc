...
def label_tokenization(line, **kwargs):
    token = tokenization.json_tokenizer(line, **kwargs)
    return [token]
...
def binarize(args: Dict, filename: str, dict: Dictionary, in_file: str, attr: str,
    ...
    res = Binarizer.binarize(filename, dict, consumer,
                             tokenize=label_tokenization if attr == 'oracle' else tokenization.json_tokenizer,
...
def main(args):
    ...
    if not args['preprocess']['tgtdict'] and os.path.exists(dict_path(args['preprocess']['target_lang'])):
    ...
    if args['preprocess']['tgtdict']:
        ...
        tgt_dict = task.build_dictionary(
            ...
            workers=args['preprocess']['workers'],
            padding_factor=args['preprocess']['padding_factor'],
            pad=None, bos=None, eos=None, unk=None,
        )
    ...
    LOGGER.info('dict_path: {}'.format(dict_path(args['preprocess']['target_lang'])))
